        ...
        <main className="min-h-screen bg-transparent flex flex-col">
            <header className="px-8 py-4 bg-white/40 dark:bg-gray-900/40 backdrop-blur-md border-b border-indigo-200/40 dark:border-gray-700/40 flex justify-between items-center sticky top-0 z-50 mt-6 shadow-sm">
                <div className="flex flex-wrap items-center gap-2 md:gap-4 w-full md:w-auto">
                    <Link
                        href="/trees"
                        className="px-4 py-3 md:px-8 md:py-5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl font-black text-xs md:text-sm uppercase tracking-widest shadow-lg shadow-indigo-500/20 flex items-center gap-3 transition-all active:scale-95 group shrink-0"
                        title="К списку деревьев"
                    >
                        <ChevronLeft size={20} className="group-hover:-translate-x-1 transition-transform md:w-6 md:h-6" />
                        <span className="inline">К деревьям</span>
                    </Link>
                    ...
                    <nav className="flex items-center gap-1 overflow-x-auto no-scrollbar md:overflow-visible pb-1 md:pb-0">
                        <button onClick={() => window.location.reload()} className="p-2 md:p-4 text-gray-500 hover:text-indigo-600 flex items-center gap-2 text-xs md:text-sm font-black uppercase tracking-tight transition-colors whitespace-nowrap">
                            <RefreshCw size={20} className="md:w-6 md:h-6" />
                            <span className="hidden sm:inline">Обновить</span>
                        </button>
                        <button onClick={handleCloneTree} className="p-2 md:p-4 text-gray-500 hover:text-indigo-600 flex items-center gap-2 text-xs md:text-sm font-black uppercase tracking-tight transition-colors whitespace-nowrap">
                            <Copy size={20} className="md:w-6 md:h-6" />
                            <span className="hidden sm:inline">Копировать</span>
                        </button>
                        <button onClick={() => setIsAccessModalOpen(true)} className="p-2 md:p-4 text-gray-500 hover:text-indigo-600 flex items-center gap-2 text-xs md:text-sm font-black uppercase tracking-tight transition-colors whitespace-nowrap">
                            <Share2 size={20} className="md:w-6 md:h-6" />
                            <span className="hidden sm:inline">Доступ</span>
                        </button>
                        <button onClick={handleResetLayout} className="p-2 md:p-4 text-gray-500 hover:text-indigo-600 flex items-center gap-2 text-xs md:text-sm font-black uppercase tracking-tight transition-colors whitespace-nowrap">
                        ...
                        <button
                            ...
                            className="p-2 md:p-4 text-gray-500 hover:text-indigo-600 flex items-center gap-2 text-xs md:text-sm font-black uppercase tracking-tight transition-colors disabled:opacity-50 whitespace-nowrap"
                        >
                            {isExporting ? <RefreshCw size={20} className="animate-spin md:w-6 md:h-6" /> : <Printer size={20} className="md:w-6 md:h-6" />}
                            <span className="hidden sm:inline">PDF</span>
                        </button>
                        <a
                            href={`/api/tree/export-gedcom?treeId=${tree.id}`}
                            download
                            className="p-2 md:p-4 text-gray-500 hover:text-indigo-600 flex items-center gap-2 text-xs md:text-sm font-black uppercase tracking-tight transition-colors whitespace-nowrap"
                        >
                            <FileDown size={20} className="md:w-6 md:h-6" />
                            <span className="hidden sm:inline">GEDCOM</span>
                        </a>
                        <button
                            onClick={handleTreeDelete}
                            className="p-2 md:p-4 text-rose-300 hover:text-rose-600 flex items-center gap-2 text-xs md:text-sm font-black uppercase tracking-tight transition-all whitespace-nowrap"
                        >
                ...
                <div className="flex items-center gap-2 md:gap-4 ml-auto md:ml-0">
                    <Link
                        ...
                        className="px-4 py-3 md:px-8 md:py-5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl font-black text-xs md:text-sm uppercase tracking-widest shadow-xl shadow-indigo-500/20 flex items-center gap-3 transition-all active:scale-95 whitespace-nowrap"
                    >
                        <Plus size={20} className="md:w-6 md:h-6" />
                        <span className="inline">Добавить персону</span>
                    </Link>
